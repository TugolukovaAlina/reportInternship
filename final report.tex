...
However, the suggested $k$ did not coincide with the $k$ found with the experimentations. In the scenario when values $Y_1, Y_2, ..., Y_n$ have correlation $corr(Y_i, Y_{i+h}) = \rho^h$. However when we walk on the graph the values $Y_1$ and $Y_100$ can appear to be of the immediate neighbors. And when in reality the values of the corresponding nodes can be pretty much correlated, according to the geometrical model their correlation is so small that can be neglected.

\myworries{put picture where is suggested k with this method does not coinc with exper results !!! the one with facebook}

Therefore, the model is too easy to use. In the next section we will generalize it on any graph with any values.
