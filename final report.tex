...
[describe more these methods or remove the next paragraph]
There are several existing solutions for sampling hidden population such as snowball sampling, targeted sampling, time-space sampling, key-informant sampling. 
...
[write here about graph and random walk on the graph]
...
Both scenarios require the same budget, so in the terms of money they are equal. But what is better 6 more correlated samples, or 5 but less correlated samples? The better scenario is the one that will have the least error. If we are able to quantify the error depending on the number of samples that we skip $k-1$, we will be able to suggest the sampling method that will bring the most precise result. That is the question that we are going to analyze formally in the next chapter. 
...
In this way for the graph with just 100 nodes and 10 possible values it would be impossible to sample.  

In order to produce samples from this distribution we are using Gibbs sampler. The idea of Gibbs sampler is to change the configuration $\bar{x}$ with time $k$, we will denote the configuration at the time $k$ as $ \bar{x}^k $.
...
[and write about connection with energy]
When the temperature is small the values of the nodes are highly correlated. The nodes prefer to take values that minimize their local energy, the probability that the node will take value that is very low. With increasing temperature the values of the nodes become more random. The nodes still favor but , but there is also not little probability to take some other random value. 
...
- put explanation about homophily before
- rename V as P
